...
var qsgetUploadinfo = config.queryString.getUploadinfo
var qsgetErrinfo = config.queryString.getErrifo
...
router.get('/uploaded', function (req, res, next) {
    ...
    pool.connect(async function (err, client, done) {
        ...
        const query = await client.query(qsgetUploadinfo)
...
router.get('/errorInfo',function (req, res, next,) {
    ...
    pool.connect(async function (err, client, done) {
        ...
        const query = await client.query(qsgetErrinfo,[new Date(req.query.ref)])
        for (let i = 0; i < query.rows.length; i++) {
            results.push(query.rows[i])
            done()
        }
